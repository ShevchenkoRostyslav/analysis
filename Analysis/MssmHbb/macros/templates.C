...
HbbStyle style;
std::vector<std::string> list_of_systematics = {"CMS_scale_j","CMS_res_j","CMS_eff_pTonl","CMS_eff_b","CMS_PU","CMS_eff_bonl","CMS_eff_l","CMS_PDF"};
...
int main()
{
   ...
   std::map<int,TFile*> lowMF;
   for(const auto& f : mssmhbb::signal_templates){
	   lowMF[f.first] = new TFile(f.second.c_str(),"read");
   }
   ...
   //Increase size of the Labels and Titles
//   gStyle->SetLabelSize(gStyle->GetLabelSize()*1.2,"XYZ");
//   gStyle->SetTitleSize(gStyle->GetTitleSize()*1.2,"XYZ");
   gROOT->ForceStyle();
...
//   sistTemp(lowMF,"lowM");
   ...
   selectedTemplates(lowMF,"Wide peaks low",Wide_points);
   selectedTemplates(lowMF,"Narrow peaks low",Narrow_points);
   selectedTemplates(lowMF,"low",Selected_points);
...
//	style.drawStandardTitle("out");
	std::string cPrinter = mssmhbb::cmsswBase + "/src/Analysis/MssmHbb/macros/pictures/AllinOne_" + regime + "_Masses.pdf";
	c->RedrawAxis();
...
//	double xMin = 0, xMax = 0;
	...
	int i = 0, j =0;
	   for(const auto & f : inF){
		   ++i;
		   histo.push_back(GetFromTFile<TH1D>(*f.second,"templates/bbH_Mbb_VIS"));
		   for(const auto& s : list_of_systematics){
			   histo.push_back(GetFromTFile<TH1D>(*f.second,"templates/bbH_Mbb_" + s + "_VIS_13TeVDown"));
			   histo.push_back(GetFromTFile<TH1D>(*f.second,"templates/bbH_Mbb_" + s + "_VIS_13TeVUp"));
		   }
		   c1[i-1] = new TCanvas();
...
//			   xMax = h->GetXaxis()->GetXmax();
		   ...
		   if(i==1) leg->Draw();
		   std::string canva_name = mssmhbb::cmsswBase + "/src/Analysis/MssmHbb/macros/pictures/AllinOne_" + regime + "_" + std::to_string(f.first) + "_GeV_mass_point.pdf";
...
//	double xmin = 0.6, ymin = 0.5, xmax = 0.9, ymax = 0.7;
	...
	for(const auto & f : inF){
		++i;
		TH1D * first = GetFromTFile<TH1D>(*f.second,"templates/bbH_Mbb_VIS");
...
//		first->SetLineWidth(2);
		...
		for(const auto& s : list_of_systematics){
			singleH[s] = std::make_pair( GetFromTFile<TH1D>(*f.second,"templates/bbH_Mbb_" + s + "_VIS_13TeVDown"), GetFromTFile<TH1D>(*f.second,"templates/bbH_Mbb_" + s + "_VIS_13TeVUp") );
		}
...
//			text.Draw();
	...
	std::vector<string> Syst = {"eff_b","eff_l","PU"};
