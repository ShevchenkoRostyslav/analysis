...
//	map<string,RooWorkspace*> workspace, workspace_bg_nobias;
	...
	std::map<string,TH1F> h_pdf_bkg_nobias_i;
...
//				Normalization(pdf_sgn_norm_i[sr]->getVal()*data[sr].xsec_vis , RooAbsReal::NumEvent) );
	    ...
	    white_line_dn[sr] = new TLine(	h_pdf_bkg_nobias_i[sr].GetXaxis()->GetXmin(), 0, h_pdf_bkg_nobias_i[sr].GetXaxis()->GetXmin(), h_pdf_bkg_nobias_i[sr].GetBinContent(1));
		white_line_dn[sr]->SetLineColor(kWhite);
		white_line_dn[sr]->SetLineWidth(1.4);

	    white_line_up[sr] = new TLine(	h_pdf_bkg_nobias_i[sr].GetXaxis()->GetXmax(), 0, h_pdf_bkg_nobias_i[sr].GetXaxis()->GetXmax(), h_pdf_bkg_nobias_i[sr].GetBinContent(h_pdf_bkg_nobias_i[sr].GetNbinsX()));
		white_line_up[sr]->SetLineColor(kWhite);
		white_line_up[sr]->SetLineWidth(1.4);

		frame1->Draw();

	    frame1_i[sr]->addTH1(&h_pdf_bkg_nobias_i[sr],"P HIST SAME");
...
//		c1.Draw();
	...
	leg->AddEntry(frame1_i["sr1"]->findObject((h_pdf_bkg_nobias_i["sr1"].GetName())),"Background","L");
...
//	TPad pad1_0("pad1_0", "pad1", 0.02, 0.50, 1, 0.96);
	...
	can_4pads.Print( (output_path  + "AllInOne_BgOnly_log.pdf").c_str());
}
...
//     hbkg.SetLineWidth(0);
    ...
    cout<<"Plot sgn..."<<endl;
    std::cout<<"WTF: "<<pdf_sgn->createIntegral(RooArgSet(*x))->getVal()<<endl;
...
//         hbkg.SetBinError( bin_i, 0.);
		...
		cout<<"WTF: e_data = "<<e_data_i<<" sqrt(data) = "<<sqrt(n_data_i)<<" sqrt(bg) = "<<sqrt(bkg_int_i)<<" up err = "<<h_rebinned.GetBinErrorUp(bin_i)<<endl;
